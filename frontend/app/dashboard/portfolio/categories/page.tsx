...
export default function PortfolioCategoriesDashboardPage() {
  const router = useRouter();
  const base = getApiBase();
  const [list, setList] = useState<PortfolioCategory[]>([]);
  ...
  function startEdit(c: PortfolioCategory) {
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    try {
      const r = await fetch(
        `${base}/api/portfolio/categories/${encodeURIComponent(editingSlug)}/`,
        {
          method: "PATCH",
          headers: { ...authHeaders(), "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function removeCategory(slug: string, name: string) {
    ...
    const ok = window.confirm(
      `Delete category “${name}”? Projects may still list until you edit them.`,
    );
    if (!ok) return;
    setErr(null);
    setMsg(null);
    ...
    try {
      const r = await fetch(
        `${base}/api/portfolio/categories/${encodeURIComponent(slug)}/`,
